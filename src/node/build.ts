...
export async function buildBundle(root: string) {
   ...
   * @returns
   */
  const buildViteConfig = (isSsr = false): InlineConfig => {
    return {
      mode: 'production',
      root,
      build: {
...
export async function renderPage(
  ...
  // 获取客户端打包出来的js的能力
  const clientBundleFileName = clientBundle.output.find(
...
export async function build(root: string = process.cwd()) {
  ...
  // 2. 引入server-entry 模块
  const serverBundlePath = serverBundle.output.find(
    (chunk) => chunk.type === 'chunk' && chunk.name === 'server-entry'
  ).fileName
  const { render } = await import(
    pathToFileURL(join(root, '.temp', serverBundlePath)).toString()
  )
  // 3. 产出html写入磁盘
